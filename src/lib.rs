...
fn fwtype(fp: &mut dyn BufRead, csize: &CharSize,
            lineheight: usize,
            outmargin: usize, sepmargin: usize, frames: usize,
            tabstop: usize, wchars: usize,
            basedrift_a: isize,
            gridhpitch: usize, gridvpitch: usize,
            grid: bool,
            dospcmarking: bool, donumbering: bool, 
            abovegap: String, belowgap: String,
            linenumoffset: usize, linenumwidth: usize
            ) {
    let nchars;
    let mut linec;
    let mut rowc;
...
eprintln!("numbering {} width {} offset {}",
...
*/
    ...
    eprintln!("csize {}x{} lineheight {}",
        csize.width, csize.height, lineheight);
    eprintln!("nchars {} linec {} rowc {} num_overrun {} basedrift_a {} ndigts {}",
        nchars, linec, rowc, num_overrun, basedrift_a, ndigits);
/*
*/

    let numwid = if donumbering { csize.width*ndigits } else { 0 };
...
//    println!("\\tt%");
    ...
    println!("\\fontsize{{{}pt}}{{{}pt}}\\selectfont%",
...
/*
    println!("\\def\\spcmark{{\\fontsize{{{}pt}}{{{}pt}}\\selectfont$\\diamond$}}%",
        (2*csize.height/3), (2*csize.height/3) );
*/
    println!("\\def\\spcmark{{\\fontsize{{{}pt}}{{{}pt}}\\selectfont$\\triangle$}}%",
        (2*csize.height/3), (2*csize.height/3) );

    println!("\\def\\VV{{\\vrule width 0pt height 0.90em depth .25em}}%");
    println!("\\def\\FA#1#2#3{{\\put(#1,#2){{\\makebox({},{}){{\\VV\\mbox{{#3}}}}}}}}%", csize.width, csize.height);
    println!("\\def\\FX#1#2#3{{\\put(#1,#2){{\\makebox({},{}){{\\VV\\mbox{{#3}}}}}}}}%", csize.width, csize.height);
...
*/
    ...
    if frames == 0xf {
        ...
        if (frames & 0x02)>0 {
            println!(" \\put({},{}){{\\line(-1,0){{{}}}}}",
                txoffset+txwidth, cvheight, txwidth);
        }
        if (frames & 0x04)>0 {
            println!(" \\put({},{}){{\\line(0,-1){{{}}}}}",
                txoffset+txwidth, cvheight, cvheight);
        }
    }
...
println!("\\linethickness{{0.1pt}}");
        ...
        for gx in 0..=nchars {
            if gx%gridhpitch==0 {
                println!("  \\put({},{}){{\\line(0,1){{ {} }} }}",
                    txoffset+outmargin+gx*csize.width, cvheight*0, cvheight);
            }
        }
        ...
        for gy in 0..=rowc {
            if gy%gridvpitch==0 {
                println!("  \\put({},{}){{\\line(1,0){{ {} }} }}",
                    txoffset, cvheight-(outmargin+gy*lineheight), txwidth);
            }
        }
            ...
            if donumbering {
                if rinline==1 {
                    let numstr = format!("{:>width$}",
                        linenumoffset+linec+1, width=ndigits);
                    c = 0;
...
*/
                        if let Some(label) = line[p..].chars().next() {
                            ...
                            println!(" \\FA{{{}}}{{{}}}{{\\fbox{{\\hbox to 2em{{\\hss\\VV{{}}{}\\hss}}}}}}",
                                x+3*csize.width/2, (y as isize) - basedrift_a, label);
                            c = c + 3;
                            q = q + 1;
                        }
                    }
                        ...
                        println!(" \\FA{{{}}}{{{}}}{{{}}}",
                            x, (y as isize) - basedrift_a, och);
                    }
                }
                else {
                    println!(" \\FX{{{}}}{{{}}}{{{}}}",
                        x+csize.width/2, (y as isize) - basedrift_a, ch);
                    c = c + 1;
                }
                c = c + 1;
...
*/
                    ...
                    println!(" \\put({},{}){{\\line(0,1){{ {} }}}}",
                        txoffset + txwidth, y+lineheight/8, 6*lineheight/8);
                    println!("\\thinlines");
                    break;
                }
            }
...
//  dbg!(&config);
    ...
    for (_file_num, filename) in config.files.iter().enumerate() {
        match open(filename) {
            ...
            Ok(mut file) => {
                fwtype(&mut file, &config.csize, config.lineheight,
                    config.outmargin, config.sepmargin,
                    config.frames,
                    config.tabstop, config.wchars,
                    config.basedrift_a,
                    config.gridhpitch, config.gridvpitch, config.grid,
                    config.spcmarking, config.numbering, 
                    config.abovegap.clone(), config.belowgap.clone(),
                    config.linenumoffset, config.linenumwidth
                    );
            }
        }
    }
...
fn test_parse_2d_int() {
    ...
    assert!(res.is_ok());
    assert_eq!(res.unwrap(), CharSize{ height:8, width:4} );

    // 12x6 is an OK integer
    let res = parse_2d_int("12x6");
    assert!(res.is_ok());
    assert_eq!(res.unwrap(), CharSize{ height:12, width:6} );
}
